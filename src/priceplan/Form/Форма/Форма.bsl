...
&НаСервере
Функция ПолучитьЛокальнуюСсылку(пУИД, пТипСсылки, пИНН="")
	// ищем контрагента в комментариях
	...
	Если пТипСсылки = "Контрагент" Тогда
		...
		|ГДЕ
		|	Контрагенты.ДополнительнаяИнформация ПОДОБНО &Комментарий";

		Запрос			= Новый Запрос;
		Запрос.Текст	= ТекстЗапроса;
		Запрос.УстановитьПараметр("Комментарий","%PricePlan:"+пУИД+"%");

		Выборка			= Запрос.Выполнить().Выбрать();
		Если  Выборка.Следующий() Тогда
			лНайдСсылка	= Выборка.Ссылка;
			Возврат лНайдСсылка;
		КонецЕсли;

		Если ЗначениеЗаполнено(пИНН) Тогда
			ТекстЗапроса	=
			"ВЫБРАТЬ
			|	Контрагенты.ИНН,
			|	Контрагенты.Ссылка
			...
			Запрос			= Новый Запрос;
			Запрос.Текст	= ТекстЗапроса;
			Запрос.УстановитьПараметр("ИНН",пИНН);
...
Функция Прочитать_Сервер(пНижняяГраница, Знач пШаг, пВсегоЗаписей, пЭтоТест)
	...
	HTTP 	= Новый HTTPСоединение(Сервер,,Ключ,Пароль,,0);
...
Процедура ДобавитьДанныеДокументов(лСтрукОтвет)
	 ...
	 Если ТипЗнч(лСтрукОтвет) = Тип("Структура")
		...
		Для Каждого лСтрокаДок Из МасДокументов Цикл
			...
			Иначе
				НовСтрДокумент.Контрагент			= ПолучитьЛокальнуюСсылку(НовСтрДокумент.ИД, "Контрагент", НовСтрДокумент.КонтрагентИНН);
			...
			//Сообщить("1");
			//Сообщить(НовСтрДокумент.КонтрагентОГРН);
			//Сообщить(лСтрокаДок.client__variable__ogrn);
			//Сообщить("2");
...
Процедура ЗаписатьКонтрагента(пСтрокаДок)
	...
	лНайдСсылка		= ПолучитьЛокальнуюСсылку(пСтрокаДок.КонтрагентИД, "Контрагент", пСтрокаДок.КонтрагентИНН);
	Если ЗначениеЗаполнено(лНайдСсылка) Тогда
		пСтрокаДок.Контрагент	= лНайдСсылка;
		Сообщить("НАШЕЛ ССЫЛКУ");
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(пСтрокаДок.КонтрагентОГРН) Тогда
		НовКонтрагент.РегистрационныйНомер			= пСтрокаДок.КонтрагентОГРН;
		НовКонтрагент.Комментарий	= "ОГРН: " + пСтрокаДок.КонтрагентОГРН;
	КонецЕсли;

	НовКонтрагент.ДополнительнаяИнформация	= НовКонтрагент.ДополнительнаяИнформация+" PricePlan:"+пСтрокаДок.КонтрагентИД;
	...
	Если ЗначениеЗаполнено(пСтрокаДок.КонтрагентБанкСчет) Тогда
		...
		Сообщить(пСтрокаДок.КонтрагентБанкСчет);
		//НовБанкСчет.НомерСчета  = пСтрокаДок.КонтрагентБанкСчет;
...
Процедура ЗаписатьНоменклатуру(пСтрокаТЧ)
	...
	НовНоменклатура.ВидНоменклатуры         = Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Услуги");
